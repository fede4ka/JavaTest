--Следующая строчка сбрасывает таблицы, соответственно все вбитые данные (кроме тестовых) удалятся.
...
CREATE TABLE IF NOT EXISTS usr_info (user_info_id SERIAL UNIQUE,name VARCHAR,lastname VARCHAR,age INT, FOREIGN KEY (user_info_id) REFERENCES usr(id));
