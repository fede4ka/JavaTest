...
lastname VARCHAR,
 age INT,
 FOREIGN KEY (user_info_id) REFERENCES usr(id)
 );
...
--FOREIGN KEY (ig_id) REFERENCES purchases(p_id)
--CREATE TABLE IF NOT EXISTS purchases (p_id INT ,items_id INT, pdate Date ,PRIMARY KEY(items_id), FOREIGN KEY (p_id) REFERENCES usr(id));
--CREATE TABLE IF NOT EXISTS items (id INT , name VARCHAR, price double precision, count INT, FOREIGN KEY (id) REFERENCES purchases(items_id) );
